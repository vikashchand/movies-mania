...
const CustomerDetails = () => {
    ...
    try {
      ...
      for (const customerEmail of selectedCustomers) {
        ...
        if (customer && customer.status !== 'sent') {
          try {
            const response = await axios.post(`${baseUrl}mail/send-email`, {
            ...
            if (response.status === 200) {
              ...
              await axios.put(`${baseUrl}user/updatecustomerDetails/${customer._id}`, {
                status: 'sent'
              });
            ...
            await axios.put(`${baseUrl}user/updatecustomerDetails/${customer._id}`, { status: errorMessage });
          }
        }
      }
      fetchUsers();
  ...
  const fetchUsers = async () => {
    try {
      const response = await axios.get(`${baseUrl}user/customerDetails`); // Replace with your API endpoint to fetch users
  ...
  return (
    ...
    <p> Create ,Upload and Generate FIR</p>
